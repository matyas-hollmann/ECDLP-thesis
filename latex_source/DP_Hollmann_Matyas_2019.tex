...
\acknowledgements{I cannot begin to express my thanks to my thesis advisor, Ing. Ivo Petr, Ph.D., who provided me with unparalleled support and gave me valuable advice throughout this research project. I would also like to extend my gratitude to my family, who always supported me during my studies at CTU.}

\abstractEN{The elliptic curve discrete logarithm problem (ECDLP) is one of the most important problems in asymmetric cryptography. In recent years, several papers were concerned with the use of summation polynomials to solve the ECDLP efficiently. The vast majority of those papers dealt only with the composite field case. In this thesis, we summarize and test the state-of-the-art algorithms solving ECDLP by means of summation polynomials for the prime field case. Unfortunately, these algorithms are not yet practical, and more research needs to be done.}

\abstractCS{Problém diskrétního logaritmu na eliptické křivce (ECDLP) je jedním z vůbec nejdůležitějších problému v asymetrické kryptografii. Několik článku se v posledních letech zabývalo použitím sumačních polynomů pro efektivní řešení ECDLP. Drtivá většina těchto prací se však zabývala pouze případem řešení nad rozšířením konečného prvotělesa. My v této práci shrneme a otestujeme nejnovější algoritmy řešící ECDLP pomocí sumačních polynomů nad prvotělesy samotnými. Bohužel musíme konstatovat, že tyto algoritmy ještě nejsou použitelné v praxi a je tedy potřeba dalšího výzkumu v této oblasti.}
